...
const Home: NextPage = () => {
  ...
  const getAddress = async () => {
    ...
    const response = await cosmosApp!.getAddressAndPubKey(
      bip44ToArray(DEFAULT_DERIVATION_PATH_LEDGER),
      'band'
    )
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.buttonGroup}>
        <button className={styles.button} onClick={handleBTC}>
          Connect BTC
        </button>
        <button className={styles.button} onClick={handleCosmos}>
          {cosmosApp ? 'Cosmos Connected' : 'Connect Cosmos'}
        </button>
